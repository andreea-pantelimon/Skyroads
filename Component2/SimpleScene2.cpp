...
void SimpleScene2::InitResources()
{
	...
	cameraInput = new CameraInput2(camera);
	window = Engine::GetWindow();

	SceneInput2 *SI = new SceneInput2(this);
...
void SimpleScene2::AddMeshToList(Mesh * mesh)
{
	if (mesh->GetMeshID())
	{
...
void SimpleScene2::DrawCoordinatSystem()
{
	DrawCoordinatSystem(camera->GetViewMatrix(), camera->GetProjectionMatrix());
}

void SimpleScene2::DrawCoordinatSystem(const glm::mat4 & viewMatrix, const glm::mat4 & projectionMaxtix)
{
	glLineWidth(1);
...
void SimpleScene2::RenderMesh(Mesh * mesh, Shader * shader, glm::vec3 position, glm::vec3 scale)
{
...
void SimpleScene2::RenderMesh(Mesh * mesh, glm::vec3 position, glm::vec3 scale)
{
	RenderMesh(mesh, shaders["Simple"], position, scale);
}

void SimpleScene2::RenderMesh2D(Mesh * mesh, Shader * shader, const glm::mat3 &modelMatrix)
{
	if (!mesh || !shader || !shader->program)
		return;
...
void SimpleScene2::RenderMesh2D(Mesh * mesh, const glm::mat3 & modelMatrix, const glm::vec3 & color) const
{
...
void SimpleScene2::RenderMesh(Mesh * mesh, Shader * shader, const glm::mat4 & modelMatrix)
{
	if (!mesh || !shader || !shader->program)
		return;
...
void SimpleScene2::ReloadShaders() const
{
	cout << endl;
	cout << "=============================" << endl;
...
Camera * SimpleScene2::GetSceneCamera() const
{
	return camera;
}

InputController * SimpleScene2::GetCameraInput() const
{
	return cameraInput;
}
